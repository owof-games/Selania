...
namespace Febucci.TextAnimatorForUnity.Actions
{
    ...
    public abstract class TypewriterActionScriptable : ActionScriptableBase
    {
        ...
        public override IActionState CreateActionFrom(ActionMarker marker, object typewriter)
        {
            ...
            if (typewriter is MonoBehaviour mb) return new CoroutineState(this, mb);

            var dispatcher = MonoDispatcher.Instance;
            if (dispatcher != null) return new CoroutineState(this, dispatcher);

            Debug.LogWarning(
                $"TypewriterActionScriptable '{name}': Neither CreateCustomState nor PerformAction was overridden, or typewriter is not a MonoBehaviour. Action will not execute.",
                this);
            return null;
        }
        ...
        private class CoroutineState : IActionState
        {
            ...
            public ActionStatus Progress(float deltaTime, ref TypingInfo typingInfo)
            {
                ...
                if (activeCoroutine == null) activeCoroutine = runner.StartCoroutine(PerformCoroutineWrapper());
