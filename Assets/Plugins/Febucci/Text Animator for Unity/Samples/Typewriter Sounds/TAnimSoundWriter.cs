...
namespace Febucci.UI.Examples
{
    ...
    public class TAnimSoundWriter : MonoBehaviour
    {
        ...
        [SerializeField]
        [MinValue(0)]
        private float minSoundDelay = .07f;

        [Tooltip(
            "True if you want the new sound to cut the previous one\nFalse if each sound will continue until its end")]
        [SerializeField]
        ...
        private void Awake()
        {
            ...
            Assert.IsNotNull(GetComponent<TypewriterComponent>(),
                "TAnimSoundWriter: Component TAnimPlayerBase is not present");
