...
{
    - bedroom == 1:
        {charTag(witch_tag(), witch_state())}   <i>Un morbido letto è comparso, in attesa di membra stanche.</i>
...
//Gestione delle impostazioni, raggiungibile dal libro e dalla camera da letto
    + {are_two_entities_together(PG, Settings)} [Settings]
        {charTag(witch_tag(), witch_state())}   <i>Qui in futuro si potranno modificare le impostazioni.</i>
            -> main
...
=== gm_rm_activation
    {charTag(witch_tag(), witch_state())}   <i>(testo temporaneo per il testing interno) Prima di iniziare, ci chiediamo in che modalità giocare.
