...
=== book_test_intro ===
<i>La biblioteca freme all'idea di offrirti una storia da leggere.</i> #speaker:{fungus_tag()}
        + {readStories != ()} [Voglio rileggere qualcosa.]
        <i>La tua libreria contiene {number_translator(readStories)} {libro_libri(readStories)}.</i> #speaker:{fungus_tag()}
            -> reread
...
=== storyQuestions ===
    ...
    <i>La biblioteca ti offre una storia...</i> #speaker:{fungus_tag()}
...
{shuffle:
    ...
    = reread_adri_allora
    <i>La biblioteca si chiede cosa tu voglia rileggere.</i> #speaker:{fungus_tag()}
