    ...
    In particolare, ripensando alle nostre conversazioni, mi farebbe piacere parlare di più... #speaker:{PG_tag()} #inkA:offState #inkB:offState #inkC:offState  #inkD:offState #ewWord:{em_state(Other)} #portrait:PG_neutral
        {
            - not first_char_main_storylets.nine:
                {charTag(witch_tag(), witch_state())}   <i>{charNameTwo} ha altri temi che vorrebbe condividere con {player_name}, ma vorrebbe che {player_name} prima lo conoscesse un po' meglio.</i>
        }
        ...
        Dici che è pronto?#speaker:{secondChar_tag()} #inkA:{ink_tag_a(secondChar_InkLevel)} #inkB:{ink_tag_b(secondChar_InkLevel)}  #inkC:{ink_tag_c(secondChar_InkLevel)}  #inkD:{ink_tag_d(secondChar_InkLevel)} #ewWord:{em_state(Influenced)} #portrait:riccio_emotional
        
            {
                - backpack_findedGifts == ():
                    {charTag(witch_tag(), witch_state())}   <i>{player_name} non ha niente da aggiungere alla ricetta.</i>
                    -> recipe_name_creator ->
                ...
                {charTag(witch_tag(), witch_state())}   <i>Il giusto ingrediente renderà la ricetta di {player_name} e {charNameTwo} spaziale.</i>
