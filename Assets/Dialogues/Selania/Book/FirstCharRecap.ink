...

{
    - first_character_feedback: Mentore dice che <>
        ...
        {
            - firstChar_possibleStates has Triangolo: ho aiutato {translator(firstChar_ActualName)} a trovare il suo lato più pratico e razionale, ma che forse non era quello di cui aveva bisogno.
                
            - firstChar_possibleStates has RagazzaOrchestra: ora {translator(firstChar_ActualName)} ha accolto un'indole più oppositiva e attiva, forse però non qualcosa che sia nella sua natura.
                            
            - firstChar_possibleStates has FlautoDolce: probabilmente {translator(firstChar_ActualName)} sapeva già che gli affetti sono una cosa fondamentale per lei. Si chiede se è il massimo che potevamo fare.
                            
            - firstChar_possibleStates has Ocarina: ora {translator(firstChar_ActualName)} che ha accolto gioco e creatività è più felice. Ma Mentore non sembra convinta di questa scelta.
            
            - firstChar_possibleStates has Violino: abbiamo fatto un gran lavoro assieme, perché {translator(firstChar_ActualName)} aveva bisogno di riconoscere che per lei la vita ha senso solo con uno scopo più grande.
